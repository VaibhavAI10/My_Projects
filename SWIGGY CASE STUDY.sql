...
WITH TOP_RESTAURANTS_SALE AS (SELECT O.R_ID,R.R_NAME,  MONTHNAME(O.DATE) AS MONTHS , SUM(AMOUNT) AS TOTAL_AMOUNT FROM ORDERS O
								INNER JOIN RESTAURANTS R
								ON O.R_ID=R.R_ID
								INNER JOIN ORDER_DETAILS OD
								ON O.ORDER_ID=OD.ORDER_ID
								GROUP BY O.R_ID,R.R_NAME, MONTHS
                                ORDER BY TOTAL_AMOUNT DESC)
                                
SELECT * FROM TOP_RESTAURANTS_SALE R1
WHERE TOTAL_AMOUNT= (SELECT MAX(TOTAL_AMOUNT) FROM TOP_RESTAURANTS_SALE R2
						WHERE R2.MONTHS=R1.MONTHS )
...
/* OBSERVATIONS
    ...
    * IN MAY-- DOMINOS HAVE MAXIMUM SALES
      IN JUNE-- DOMINOS HAVE MAXIMUM SALES
